...
func (suite *FlaggerTestSuite) TestEmptyFlag() {
	...
	suite.Nil(suite.cli.getFlagSet(suite.cli.commandSets["root"].SubCommands["testcommand"]))
}
...
func (suite *FlaggerTestSuite) TestWrongFlag() {
	...
	suite.NotNil(suite.cli.getFlagSet(suite.cli.commandSets["root"].SubCommands["testcommand2"]))
}
...
func (suite *FlaggerTestSuite) TestFlag() {
	...
	conf := testcommand2{}
	suite.cli.Add(&conf)
	suite.Nil(suite.cli.getFlagSet(suite.cli.commandSets["root"].SubCommands["testcommand2"]))
...
type TestCommandWithSubCommand struct {
	*Flagger
	MainProgramParam bool `flag:"mainParam,string flag example"`
	subCommands      []Command
}

func (c *TestCommandWithSubCommand) Desc() string {
	return "Test description"
}
func (c *TestCommandWithSubCommand) Run(ctx context.Context) {
	fmt.Println("its okey")
}
func (c *TestCommandWithSubCommand) Samples() []string {
	return []string{"test samle"}
}
func (c *TestCommandWithSubCommand) SubCommands() []Command {
	return c.subCommands
}

func (suite *FlaggerTestSuite) TestSubcommandFlag() {
	type testcommand2 struct {
		TestCommand
		String   string        `flag:"string,string flag example"`
		Int      int           `flag:"int,int flag example"`
	...
	conf := TestCommandWithSubCommand{
		subCommands: []Command{reference2},
	}
	suite.cli.Add(&conf)

	args := []string{"testcommand", "testcommand2",
		"-string", "whales", "-int", "42",
		"-int64", "107374182400", "-uint", "7",
		"-uint64", "24", "-float64", "1.55", "-bool",
		"-duration", "15m",
	...
	_, err := suite.cli.getSubCommand(suite.cli.commandSets["root"], args)
	suite.Nil(err)
	conf.Flagger = nil
	suite.Equal(conf, conf)
}
